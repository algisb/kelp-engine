...
void Empty::update()
{
    ...
    {
        m_owner->m_world->deleteEntityNextFrame( ((World_0*)(m_owner->m_world))->wall );
    }
    if(Input::Keyboard::is(Input::Keyboard::KeyboardKey::KEY_Y, Input::Keyboard::KeyboardAction::PRESSED))
    {
        ...
        ((World_0*)(m_owner->m_world))->wall->addComponent(new Render(m_owner->m_world->m_core->m_cubeMesh, m_owner->m_world->m_core->m_shaderDefault, RenderMode::SOLID));
    }
}
void Empty::render()
{
